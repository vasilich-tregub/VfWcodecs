...
{
    if (!rlevfw_compress_query(bin, bout))
        return ICERR_OK;

    memcpy(bout, bin, bin->bmiHeader.biSize);
    bout->bmiHeader.biCompression = mmioFOURCC('R', 'L', 'E', 'f');
...
LRESULT rlevfw_compress(ICCOMPRESS* icc, size_t iccsz)
{
    ...
    *icc->lpdwFlags = AVIIF_KEYFRAME;
    rle_compress((icc->lpOutput), &outsize, icc->lpInput, sz);
...
{
    //if (!rlevfw_decompress_query(bin, bout))
        //return 0;
    ...
    bout->bmiHeader.biSizeImage = bin->bmiHeader.biWidth * bin->bmiHeader.biHeight;
...
{
    return ((bin->bmiHeader.biWidth + 15) & ~15) * ((bout->bmiHeader.biHeight + 31) & ~31) * 1 + 4096;
}
...
LRESULT rlevfw_decompress(ICDECOMPRESS* icc, size_t iccsz)
{
    ...
    rle_decompress((icc->lpOutput), &outsize, icc->lpInput, sz);
    icc->lpbiOutput->biSizeImage = (DWORD)outsize;
